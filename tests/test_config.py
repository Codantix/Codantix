...
import yaml
import pytest
from codantix.config import Config, ConfigValidationError, DocStyle, VectorDBType, LLMConfig, VectorDBConfig
...
def test_config_validation():
    ...
    assert vector_db_config.type in [VectorDBType.CHROMA, VectorDBType.QDRANT, VectorDBType.MILVUS, VectorDBType.MILVUS_LITE]
...
def test_config_constructor_all_args():
    ...
    vector_db = VectorDBConfig(type="qdrant", path="/tmp/vecdb", provider="openai", embedding="text-embedding-ada-002", dimensions=1536)
    config = Config(
...
def test_config_property_accessors():
    config = Config(doc_style=DocStyle.NUMPY, source_paths=["foo"], languages=["python"])
