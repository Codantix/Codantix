...
class RateLimitConfig(BaseModel):
    ...
    """
    llm_requests_per_second: float = Field(0.1, description="Max LLM requests per second")
    llm_check_every_n_seconds: float = Field(0.1, description="How often to check if a request can be made (seconds)")
    llm_max_bucket_size: int = Field(10, description="Maximum burst size for LLM requests")
...
class LLMConfig(BaseModel):
    ...
    stop_sequences: List[str] = Field(default_factory=lambda: [], description="Stop sequences for the LLM")
...
class VectorDBConfig(BaseModel):
    ...
    embedding: str = Field("thenlper/gte-base", description="Embedding model name")
    ...
    def check_vector_db_type(self):
        ...
        if self.type not in valid_types:
            raise ConfigValidationError(f"Unsupported vector_db type: {self.type}. Must be one of: {valid_types}")
        return self
class Config(BaseModel):
    ...
    source_paths: list[str] = Field(default_factory=lambda: ["src"], description="Paths to source code files")
    languages: list[str] = Field(default_factory=lambda: ["python", "javascript", "java"], description="Programming languages to analyze")
    ...
    def check_vector_db_type(self):
        ...
        if self.doc_style not in valid_types:
            raise ConfigValidationError(f"Unsupported doc_style: {self.doc_style}. Must be one of: {valid_types}")
        
        is_valid_language = [lang for lang in self.languages if lang in LANGUAGE_EXTENSION_MAP.keys()]
        if not is_valid_language:
            raise ConfigValidationError(f"Unsupported language: {self.languages}. Must be one of: {LANGUAGE_EXTENSION_MAP.keys()}")
        return self
    ...
    def load(cls, config_path: Optional[str] = None) -> "Config":
        ...
        """
        path = config_path or "codantix.config.json"
        try:
    ...
    def save(self, path: Optional[str] = None, format: str = "json") -> None:
        ...
        data = enum_to_value(self.dict(exclude={"config_path"}))
    ...
    def get_doc_style(self) -> str:
        return self.doc_style.value if isinstance(self.doc_style, DocStyle) else self.doc_style
